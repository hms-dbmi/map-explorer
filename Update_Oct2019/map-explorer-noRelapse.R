...
choices_five <- unique(nfive_mss_withRl$PatientNum) 
...
drills <- reactiveValues(
  current_yr = NULL,       #category
  current_month = NULL     #sub_category
)
...
output$history <- renderUI({
  ...
  id <- match(input$patient_num2, choices_five)
  pat_encounter <- which(nfive_mss_withRl$PatientNum == choices_five[id])
  
  # keep_category <- unique(three_mss$Category)[unique(three_mss$Category) %in% input$enco_type2]
  keep_category <- unique(nfive_mss_withRl$Category)[unique(nfive_mss_withRl$Category) %in% input$enco_type2]
  
  # k <- three_mss[pat_encounter,] %>% filter(Category %in% keep_category)
  k <- nfive_mss_withRl[pat_encounter,] %>% filter(Category %in% keep_category)
...
observe({
  ...
  if(!is.null(input$click) & first_click==F){
    ...
    # appendTab(inputId = "nav",
    #           tabPanel("Detailed MS Daily", value='de_tab2', # `value='de_tab2` has no function here.
    #                    h3("Detailed MS Daily"),
    #                    column(width = 6,
    #                           selectInput(inputId="patient_vd_num",
    #                                       label="Select Patient Number: ",
    ...
    #                           selectInput(inputId="enco_vd_type",
    #                                       label="Select Encounter type(s): ",
    #                                       choices=unique(three_mss$Category), multiple = F,
    #                                       selected = unique(three_mss$Category))),
    #                    br(), br(), br(), br(), br(), br(), br(),
    #                    h4("Encounters by Day"),
    #                    plotlyOutput("all_six", height = 650),
    ...
    ##### ! new
    appendTab(inputId = "nav",
              tabPanel("Detailed MS",   # the displayed tab name
                       column(width = 6,
                              h3("MS Patients with no Relapse"),
                              selectInput(inputId="patient_num2",
                                          label="Select Patient Number: ",
                                          choices=choices_five, #choices=choices,
                                          selected = 1),
                              # get rid of the extra line between two checkboxes
                              tags$style(".shiny-input-container {margin-bottom: 0px} .checkbox { margin-top: 0px; margin-bottom: 0px }"),
                              checkboxInput("show_stackbar2","Show stacked bar charts ",FALSE),
                       ...
                       column(width = 6,br(),
                              ...
                              selectInput(inputId="enco_type2",
                                          label="Select Encounter type(s): ",
                                          choices=levels(factor(n_order_Category))[c(1,5:12,2:4)], multiple = T,
                                          selected = levels(factor(n_order_Category))[c(1,5:12,2:4)]
                              )
                       ),
                       br(), br(), br(), br(), br(), br(), br(), br(), br(),br(),br(),
                       uiOutput("history"),
...
## for opening a new page in the browser?
observe({
