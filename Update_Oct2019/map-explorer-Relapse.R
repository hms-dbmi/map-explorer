...
choices_five <- unique(five_mss_withRl$PatientNum) 
...
output$history <- renderUI({
  ...
  pat_encounter <- which(five_mss_withRl$PatientNum == choices_five[id])
  
  # keep_category <- unique(three_mss$Category)[unique(three_mss$Category) %in% input$enco_type2]
  keep_category <- unique(five_mss_withRl$Category)[unique(five_mss_withRl$Category) %in% input$enco_type2]
  
  # k <- three_mss[pat_encounter,] %>% filter(Category %in% keep_category)
  k <- five_mss_withRl[pat_encounter,] %>% filter(Category %in% keep_category)
    ...
    appendTab(inputId = "nav",
              tabPanel("Detailed MS",   # the displayed tab name
                       ...
                       column(width = 6,br(),
                              ...
                              selectInput(inputId="enco_type2",
                                          label="Select Encounter type(s): ",
                                          choices=levels(factor(order_Category))[c(1,5:12,2:4)], multiple = T,
                                          selected = levels(factor(order_Category))[c(1,5:12,2:4)]
                                          )
                              ),
                       br(), br(), br(), br(), br(), br(), br(), br(), br(),br(),br(),
                       uiOutput("history"),
